...
g_url_queue.put('http://buptoa.bupt.edu.cn/broad.nsf/($All)?OpenView')
tt = ['http://buptoa.bupt.edu.cn/broad.nsf/($All)?OpenView']
g_url_set = set(tt)
max_deep = 1

root_url = 'http://buptoa.bupt.edu.cn'


def is_good_url(str):
    patt = 'view'
    m = re.search(patt,str) #注意mathch 和 search 的区别 math 要全部，search 部分
    if m is None:
        return 1
    else:
        return 0
...
def get_url_list(html):
    ...
    for x in res:
        ...
        for xx in m:
            ...
            str_url = xx
            str_url =  root_url + str_url
            print str_url
            print 'end'
            if is_good_url(str_url) == 0:
                continue
            #print str_url
